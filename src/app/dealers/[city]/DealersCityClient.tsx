...
export default function DealersCityClient({ city }: { city: string }) {
  const cityName = capitalize(city);

  const { data, isLoading } = useApi(
    () => fetchDealerSearch({ city, limit: 20 }),
    [city]
  );
          ...
          <div className="space-y-3">
            {dealers.map((dealer) => (
              ...
              >
                ...
                <div className="flex gap-2 mt-3">
                  ...
                  <a
                    href={`https://maps.google.com/?q=${encodeURIComponent(dealer.dealershipName + " " + cityName)}`}
                    target="_blank"
            ...
            {dealers.length === 0 && (
              <div className="text-center py-12">
                ...
                <p className="text-slate-400 text-sm">No dealers found in {cityName} yet.</p>
                <Link href="/dealers" className="text-blue-400 text-xs mt-2 inline-block">
                  Browse all dealers
                </Link>
              </div>
            )}
