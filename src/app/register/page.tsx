...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => router.push("/login/buyer"), 1500);
    } catch {
  ...
  return (
    <div className="min-h-screen bg-[#080a0f] text-[#e2e8f0]">
      <div className="max-w-lg mx-auto px-4 pb-32">
        ...
        {success ? (
          <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 px-5 py-6 text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">Check your email to verify, then sign in.</p>
          </div>
        ) : (
          <>
            {error && (
              <div className="rounded-lg border border-red-400/20 bg-red-400/10 px-4 py-3 text-xs text-red-400 mb-4">
                {error}
              </div>
            )}
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-3 bg-white/5 border border-white/10 hover:bg-white/10 rounded-xl py-3.5 transition disabled:opacity-50"
            >
