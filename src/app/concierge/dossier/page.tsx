...
function DossierContent() {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="size-10 rounded-full bg-slate-800 overflow-hidden border border-[#3B82F6]/20 flex items-center justify-center">
              <MaterialIcon name="smart_toy" className="text-[#3B82F6] text-xl" />
            </div>
          ...
          <div>
            ...
            <p className="text-[11px] uppercase tracking-widest text-[#3B82F6] font-medium mt-1">AI Generated</p>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-6 pb-32">
        <div className="rounded-xl overflow-hidden bg-slate-800/50 border border-slate-700 shadow-xl">
          <div className="relative aspect-[16/9] w-full overflow-hidden">
            ...
            <div className="absolute top-3 right-3 flex items-center gap-1 bg-[#3B82F6] px-2 py-1 rounded text-[10px] font-bold text-white uppercase">
          ...
          <div className="p-4">
            ...
            <div className="flex items-baseline gap-3 mt-3">
              <span className="text-2xl font-bold text-[#3B82F6]">{price}</span>
            </div>
          </div>
        </div>

        <div className="rounded-xl bg-slate-800/30 border border-slate-700/50 p-4">
          <h4 className="text-xs font-bold uppercase tracking-widest text-[#3B82F6] mb-4 flex items-center gap-2">
        ...
        <div className="rounded-xl p-4 border" style={{ background: "rgba(23,115,207,0.05)", borderColor: "rgba(23,115,207,0.2)" }}>
          <div className="flex items-center gap-3">
            <div className="size-12 rounded-full bg-[#3B82F6]/10 flex items-center justify-center">
              <MaterialIcon name="shield" className="text-[#3B82F6] text-2xl" />
            </div>
            <div>
              ...
              <div key={item.date} className="flex gap-4">
                <div className="flex flex-col items-center">
                  <div className="size-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center">
                    <MaterialIcon name={item.icon} className="text-sm text-[#3B82F6]" />
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-[450px] mx-auto z-30 p-4 bg-[#0A1628]/95 backdrop-blur-md border-t border-slate-800 md:hidden">
        <div className="flex gap-3">
          <Link
            href={vehicleId ? `/reservation?vehicleId=${vehicleId}` : "/reservation"}
            className="flex-1 bg-[#3B82F6] text-white py-3.5 rounded-xl flex items-center justify-center gap-2 font-bold text-sm"
          >
...
export default function ConciergeDossierPage() {
  return (
    <Suspense fallback={<div className="min-h-dvh flex items-center justify-center bg-[#0A1628]"><div className="w-8 h-8 rounded-full border-2 border-[#3B82F6] border-t-transparent animate-spin" /></div>}>
      <DossierContent />
    </Suspense>
  );
}
