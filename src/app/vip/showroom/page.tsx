...
/* Stitch: vip_digital_showroom — #E5C158 (gold), Space Grotesk, #0A1628 */
...
export default function VIPShowroomPage() {
  ...
  if (isLoading) {
    return (
      <div className="relative h-dvh w-full max-w-md mx-auto overflow-hidden flex items-center justify-center" style={{ background: "#0A1628" }}>
        <div className="flex flex-col items-center gap-3">
          <div className="size-10 rounded-full animate-pulse" style={{ background: "rgba(218,184,11,0.2)" }} />
          <span className="text-[#E5C158] text-sm font-bold tracking-widest uppercase animate-pulse">Loading VIP Showroom…</span>
        </div>
      </div>
    );
  }
  ...
  if (vehicles.length === 0) {
    return (
      <div className="relative h-dvh w-full max-w-md mx-auto overflow-hidden flex flex-col items-center justify-center gap-4" style={{ background: "#0A1628" }}>
        <MaterialIcon name="diamond" className="text-5xl text-[#E5C158]/30" />
        <p className="text-slate-400 text-sm">No VIP vehicles available</p>
        <Link href="/used-cars" className="text-[#E5C158] font-bold text-sm">Browse Cars →</Link>
      </div>
    );
  }
    ...
    >
      ...
      <header className="absolute top-0 left-0 right-0 z-30 px-6 pt-12 pb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[#E5C158] font-bold text-sm tracking-[0.3em] uppercase">VIP Showroom</span>
        </div>
          ...
          <section key={car.id} className="relative h-dvh w-full snap-start snap-always flex flex-col">
            ...
            <div className="relative z-10 mt-auto px-6 pb-40">
              <div className="inline-flex items-center gap-1 px-3 py-1 rounded-full mb-4 text-[10px] font-bold uppercase tracking-widest"
                style={{ background: "rgba(218,184,11,0.15)", color: "#E5C158", border: "1px solid rgba(218,184,11,0.3)" }}>
              ...
              <p className="text-2xl font-bold text-[#E5C158] mt-3">{car.priceDisplay}</p>
              ...
              <div className="flex gap-3 mt-6">
                ...
                >
                  <MaterialIcon name="360" className="text-[#E5C158]" /> AI Virtual Tour
                </Link>
                <Link
                  href={`/showcase/${car.id}`}
                  className="flex-1 bg-[#E5C158] text-[#0A1628] py-3.5 rounded-xl flex items-center justify-center gap-2 font-bold text-sm shadow-lg shadow-[#E5C158]/20"
                >
      ...
      <div className="absolute right-4 top-1/2 -translate-y-1/2 z-20 flex flex-col gap-3">
        {vehicles.map((_, i) => (
          <div key={i} className={`w-2 rounded-full transition-all ${i === 0 ? "h-6 bg-[#E5C158]" : "h-2 bg-white/30"}`} />
        ))}
      </div>

      {/* Bottom Nav */}
      <nav className="absolute bottom-0 left-0 right-0 z-30 border-t px-6 pb-6 pt-3 md:hidden"
        style={{ background: "rgba(10,22,40,0.9)", backdropFilter: "blur(16px)", borderColor: "rgba(218,184,11,0.1)" }}>
        <div className="flex justify-between items-center">
          ...
          <Link href="/vip/showroom" className="flex flex-col items-center gap-1 text-[#E5C158]">
